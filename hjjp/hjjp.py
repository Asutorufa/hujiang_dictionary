...
def word_simple(tree):
    ...
    for voice in tree.cssselect(css_word_prnounces):
        word_prnounces.append(
            word_text[i] + '|假名:' + voice.cssselect(css_word_prnounces_kata)[0].text_content() + '|罗马音:' +
            voice.cssselect(css_word_prnounces_kata)[1].text_content())
        i += 1
...
def word_simple_markdown(tree):
    ...
    for voice in tree.cssselect(css_word_prnounces):
        word_prnounces.append(
            '**' + word_text[i] + '**  \n假名:' + voice.cssselect(css_word_prnounces_kata)[0].text_content() + '|罗马音:' +
            voice.cssselect(css_word_prnounces_kata)[1].text_content() + '  ')
        i += 1
